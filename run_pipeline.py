...
class PipelineOrchestrator:
    ...
    def __init__(self, base_output_dir: Path = Path('out2')):
        ...
        self.code5_output = self.base_output_dir / 'code5_output'
        self.code6_output = self.base_output_dir / 'code6_output'
        self.shap_output = self.base_output_dir / 'code_shap_full'
    ...
    def step8_supp_results_summary(self, clinical_file: Optional[Path] = None) -> bool:
        ...
        # ------------------------------
        # Step 8 input auto-resolution
        # ------------------------------
        step5_candidates = [
            self.base_output_dir / 'code5_output',
            self.code3_output / 'clinical_factors_analysis',
        ]
        
        step8_input = None
        for d in step5_candidates:
            if d.exists():
                step8_input = d
                break
        
        if step8_input is None:
            logger.error("Step-8 cannot find Step-5 outputs. Tried:")
            for d in step5_candidates:
                logger.error(f"  - {d}")
            return False
        
        logger.info(f"Step-8 using clinical factors directory: {step8_input}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Orchestrate complete py_ntcpx v1.0 pipeline',
