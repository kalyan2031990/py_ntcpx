...
- **ML CV-AUC in reports** — saved to ml_validation.xlsx, included in ntcp_results (v3.0.1)
- **QUANTEC-RS numerical stability** — gEUD fallback, no NaN outputs (v3.0.1)
...
- **Function**: Comprehensive NTCP analysis
  ...
  - Uncertainty quantification
- **Output**: `ntcp_results.xlsx`, `ml_validation.xlsx` (CV-AUC, v3.0.1), models (PKL), plots (PNG)
- **Status**: Working
- **v3.0.1**: ML CV-AUC saved; QUANTEC-RS numerically stable with gEUD fallback
...
## v3.0.1 Fixes (February 2025)

| Fix | Description |
|-----|-------------|
| **ML CV-AUC** | `save_ml_validation_results()` writes CV-AUC to `ml_validation.xlsx`; ntcp_results Summary/Performance include CV-AUC columns |
| **QUANTEC-RS NaN** | RS Poisson numerical stability; gEUD-based fallback when DVH calc fails; DVH column support (Volume[%]) |
| **gEUD verification** | Formula and parameters verified; docstring updated |
