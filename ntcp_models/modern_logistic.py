...
class ModernLogisticNTCP:
    ...
    def prepare_features(self, organ_data, clinical_data=None):
        ...
        if clinical_features and clinical_data is not None:
            # Merge on PrimaryPatientID
            merge_key = 'PrimaryPatientID' if 'PrimaryPatientID' in organ_data.columns else 'PatientID'
            if merge_key in clinical_data.columns:
                X = pd.merge(
                    X,
